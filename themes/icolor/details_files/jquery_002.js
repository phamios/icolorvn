...
;(function($) {
    ...
    $.fn.oneByOne = function(options) {
		...
		function tweenTo(n){
			...
			var cursl = bannerArr[n];
			if (cursl != undefined) {
			    var id_slide = $(cursl).attr('id'); //cursl.context.attributes[1].value;
			    UpdateNumber(id_slide);
			}
		...
		function UpdateNumber(id) {
		    //alert(slides[id].ts);
		    $('.wrapper-gallery .gal .video').attr('data-slide', id);
		    $('.wrapper-gallery .gal .galcollection').attr('data-slide', id);

		    if ((totalA + totalV) <= 4) {
		        $('.wrapper-gallery .gal div').children('.border').removeClass('border-active');
		        $('.wrapper-gallery .gal div').each(function () {
		            if ($(this).attr('id') != undefined)
		            {
		                if (slides[id].lvid.indexOf($(this).attr('id')) >= 0)
		                    $(this).children('.border').addClass('border-active');
		                    
		            }
		            if ($(this).attr('data-img') != undefined) {
		                if (slides[id].lmid.indexOf($(this).attr('data-img')) >= 0) {
		                    $(this).children('.border').addClass('border-active');
		                }
		            }
		        });
		    ...
		    {
		        GetNameAlbumVideoRelateBySlide(id);
		    }

$(document).ready(function () {
    ...
    // thêm navigator vào slider
    var navCount = $(bannerWrapper).find('.img').length;
    var navHtml = '<ul id="slidernavigator">';
    for (var i = 0; i < navCount; i++) {
        navHtml += '<li data-pos="' + i + '">&nbsp;</li>';
    }
    navHtml += '</ul>';
    $('#banner').append(navHtml);
    var bannerNavigator = $('#slidernavigator');
    ...
    // vị trí hiện tại trên navigator
    var firstNav = (currentPosition == totalItem - 1) ? 0 : currentPosition - 1;
    $(bannerNavigator).find('li').eq(firstNav).addClass('active');
    ...
    $('#banner .banner-wrapper .arrow-wrapper .left-arrow').click(function (event) {
        ...
        if (leftArrowState == 0) {
            ...
            // vị trí hiện tại trên navigator
            $(bannerNavigator).find('li.active').removeClass('active');
            var nextNav = (currentPosition == totalItem - 1) ? 0 : currentPosition - 1;
            $(bannerNavigator).find('li').eq(nextNav).addClass('active');
    ...
    $('#banner .banner-wrapper .arrow-wrapper .right-arrow').click(function (event) {
        ...
        if (rightArrowState == 0) {
            ...
            var newLeftIndent = defaultLeftIndent + (itemWidth * currentPosition * -1);
            // vị trí hiện tại trên navigator
            $(bannerNavigator).find('li').removeClass('active');
            var nextNav = (currentPosition == totalItem - 1) ? 0 : currentPosition - 1;
            $(bannerNavigator).find('li').eq(nextNav).addClass('active');
    ...
    // nhấn vào icon trên navigator
    $(bannerNavigator).find('li').click(function (event) {
        event.preventDefault();
        ...
        currentPosition = $(this).data('pos') + 1;
        var newLeftIndent = defaultLeftIndent + (itemWidth * currentPosition * -1);
        window.clearInterval(slideTimer);
        $(bannerWrapper).animate({
            'left': newLeftIndent
